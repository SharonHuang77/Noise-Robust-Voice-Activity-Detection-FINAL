...
# ============================================================
# 05 - Train Baseline MLP on Clean LibriSpeech Speech
# ============================================================
# This script trains a baseline MLP model for frame-level VAD.
# The model is trained ONLY on clean speech (no noise augmentation).
...
SAVE_PATH="${SAVE_PATH:-$PROJECT_ROOT/artifacts/baseline_mlp_best.pt}"
...
if [[ ! -d "$DATA_ROOT/train/features" ]]; then
  echo "ERROR: Training features directory not found:"
  echo "  $DATA_ROOT/train/features"
  echo "Please run step 04 (extract_features.sh) first."
  exit 1
fi

# Check that clean manifests exist
if [[ ! -f "$DATA_ROOT/train/manifests/train_manifest.jsonl" ]]; then
  echo "ERROR: Clean train manifest not found:"
  echo "  $DATA_ROOT/train/manifests/train_manifest.jsonl"
  echo "Please ensure features were extracted with --manifest_type clean"
  exit 1
fi
...
# ============================================================
echo "========================================"
echo "Baseline MLP Training (Clean Speech Only)"
echo "========================================"
...
if [[ "$RUN_SWEEP" == "1" ]]; then
    ...
    for wd in "${WD_VALUES[@]}"; do
      for dr in "${DROPOUT_VALUES[@]}"; do
        for seed in "${SEED_VALUES[@]}"; do
          run_idx=$((run_idx + 1))
          run_name="baseline_mlp_lr${lr}_wd${wd}_dr${dr}_seed${seed}"
  ...
  echo
  echo "Done training baseline MLP model."
fi
